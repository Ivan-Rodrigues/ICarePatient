...
public class S2PAService extends Service implements TechnologyListener {
    ...
    /** random number for simulated sensor value*/
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // Simulating Bloodpressure
        new Thread(new Runnable() {
            @Override
            public void run() {
                simulatedSensor("00000000-0451-4000-b000-000000000023",-29.0,"zephyrbloodpressure",69.0,205.0);

            }
        }).start();

        // Simulating BPM sensor
        new Thread(new Runnable() {
            @Override
            public void run() {
                simulatedSensor("00000000-af0d-4a7a-8f9c-4fa7cf282003",-29.0,"zephyrbpm",37.0,134.0);
            }
        }).start();

        // Simulating respiratory sensor
        new Thread(new Runnable() {
            @Override
            public void run() {
                simulatedSensor("00000000-fce6-42fb-aadc-5acd91a69b1a",-29.0,"zephyrrespiratory",8.0,33.0);
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                simulatedSensor("00000000-08d5-42fb-aadc-db714f31a669",-29.0,"zephyrtemperature",34.0,40.0);
       ...
       //Caso de Uso CURB-65
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                simulatedSensor("00000000-fce6-42fb-aadc-5acd91a69b1a",-29.0,"zephyrrespiratory",25.0,35.0);
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                simulatedSensor("00000000-0451-4000-b000-000000000023",-29.0,"zephyrbloodpressure",84.0,125.0);
    ...
    public void simulatedSensor(String uuid, Double signal, String sensorName, Double lowValue, Double highValue )
    {
        while(true) {
            int aux = generator.nextInt(2);
             //AppUtils.logger('i', TAG, ">> simulatedSensor: ");

            SensorData sensorData = new SensorData();
            sensorData.setMouuid(uuid);
            sensorData.setSignal(signal);
            sensorData.setSensorName(sensorName);
            if (aux == 0) {
                sensorData.setSensorValue(new Double[]{lowValue});
            }else{
                sensorData.setSensorValue(new Double[]{highValue} );
            }
            sensorData.setAction(SensorData.READ);
            sensorData.setPriority(LocalMessage.LOW);
            // "|" => MHubMessage.SEPARATOR
            sensorData.setRoute(ConnectionService.ROUTE_TAG + "|" + MEPAService.ROUTE_TAG );//+ "|" + MPersistence.ROUTE_TAG);
